...
declare global {
  interface Window {
    ...
    __crazycraftTrackMetaLead?: (eventId: string) => boolean;
  }
}
...
function getGlobalState(): MetaPixelGlobalState {
  ...
  const state = window.__crazycraftMetaPixelState;
  state.pendingLeadEventIds ??= [];
  state.sentLeadEventIds ??= new Set<string>();
  return state;
}

const META_EVENT_ID_REGEX =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

/**
 * Queues one consent-gated browser Lead conversion. The shared event ID
 * is generated by the trusted Server Action and returned only after the
 * inquiry plus its CAPI outbox row commit successfully.
 */
export function trackMetaLead(eventId: string): boolean {
  if (!META_EVENT_ID_REGEX.test(eventId)) return false;

  const state = getGlobalState();

  if (
    state.permanentlyDisabled ||
    !state.desiredConsent ||
    state.desiredPixelId === null
  ) {
    return false;
  }

  if (
    state.sentLeadEventIds.has(eventId) ||
    state.pendingLeadEventIds.includes(eventId)
  ) {
    return true;
  }

  state.pendingLeadEventIds.push(eventId);
  flushIfReady(state);
  return true;
}

if (typeof window !== "undefined") {
  window.__crazycraftTrackMetaLead = trackMetaLead;
}
...
function flushIfReady(state: MetaPixelGlobalState) {
  ...
  while (state.pendingLeadEventIds.length > 0) {
    const eventId = state.pendingLeadEventIds.shift();
    if (!eventId || state.sentLeadEventIds.has(eventId)) continue;

    fbq("track", "Lead", {}, { eventID: eventId });
    state.sentLeadEventIds.add(eventId);
  }
}
...
export function MetaPixel() {
  ...
  const validPixelId = isValidMetaPixelId(rawPixelId) ? rawPixelId : null;
